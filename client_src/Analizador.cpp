...
void Analizador::verificarIngreso(const char& col, const char& fil){
...
bool Analizador::partidaFinalizada(const std::string& tablero){
  if (tablero.find("Felicitaciones! Ganaste!\n")!=std::string::npos ||
      tablero.find("Has perdido. Segui intentando!\n")!=std::string::npos ||
      tablero.find("La partida ha terminado en empate\n")!=std::string::npos) {
    return true;
  }
  return false;
}
